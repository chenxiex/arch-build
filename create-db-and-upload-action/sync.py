...
def get_pkg_infos(file_path: str) -> list["PkgInfo"]:
    ...
    for pkg_desc in pkgs:
        ...
        if filename and pkgname and version:
...
def download_local_miss_pkgs(
    local_packages: list["PkgInfo"],
    remote_packages: list["PkgInfo"],
    old_packages: list["PkgInfo"],
):
    local_package_files = [i.filename for i in local_packages]
    remote_package_files = [i.filename for i in remote_packages]
    old_package_files = [i.filename for i in old_packages]
    download_package_files = [i for i in remote_package_files if i not in old_package_files and i not in local_package_files]
    for r in download_package_files:
...
if __name__ == "__main__":
    ...
    download_local_miss_pkgs(local_packages, remote_packages, old_packages)